...
4. **Groq API:** The server streams the prompt to the Google Groq API.
5. **Server Streaming Response:** As Groq responds, the Express server pipes the chunks back to the client via Server-Sent Events (SSE) or raw stream.
...
- **Backend (Server)**: Node.js/Express REST API. It handles business logic, database operations (MongoDB/Mongoose), and AI integrations (Groq).
